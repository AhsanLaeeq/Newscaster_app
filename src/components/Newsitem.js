...
const NewsItem = (props) => {
  const { title, description, imageurl, go, publishedAt, author, source } = props;
    ...
    <div>
      <div className="card mb-3">
        <span
          ...
          style={{ left: '79%', top: '10px', zIndex: 1 }}
        >
          {source}
        </span>
        <img src={imageurl} className="card-img-top" alt="..." />
        <div className="card-body">
          <h5 className="card-title">{title}</h5>
          <p className="card-text">{description}</p>
          <a href={go} className="btn btn-dark" target="_blank" rel="noopener noreferrer">
            Preview
          </a>
          <p className="card-text">
            <small className="text-body-secondary">
              Author {!author ? 'Unknown' : author} on {new Date(publishedAt).toGMTString()}
            </small>
          </p>
        </div>
      </div>
